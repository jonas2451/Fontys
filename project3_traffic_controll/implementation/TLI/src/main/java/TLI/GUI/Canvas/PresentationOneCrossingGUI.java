...
public class PresentationOneCrossingGUI extends Application {

    private final static AbstractPedCrossingFxComposer COMPOSER = GermanPedCrossingFxComposer.getInstance();
    //private final static AbstractPedCrossingFxComposer COMPOSER = DutchPedCrossingFxComposer.getInstance();
...
//        );
        ...
        //<editor-fold defaultstate="collapsed" desc="two crossings">
        //        Travelable t1 =
//                SPCConnector.connectTravelablesHorizontally(
...
//        FxLane f4 = new FxLane(t4);
        ...
        //<editor-fold defaultstate="collapsed" desc="one crossing">
        Travelable t1 =
                SPCConnector.connectTravelablesHorizontally(
                        ((SimplePedestrianCrossing) eastIntersection.getIntersection()).getWestInboundRoad().getCenter().get(0),
                        new Position(0, ((SimplePedestrianCrossing) eastIntersection.getIntersection()).getWestInboundRoad().getCenter().get(0).getPosition().getY()),
                        speedLimit
                );
        ...
        Travelable t4 = SPCConnector.connectTravelablesHorizontally(
                ((SimplePedestrianCrossing) eastIntersection.getIntersection()).getEastInboundRoad().getCenter().get(0),
                new Position(0, ((SimplePedestrianCrossing) eastIntersection.getIntersection()).getEastInboundRoad().getCenter().get(0).getPosition().getY()),
                speedLimit
        );
...
//        System.out.println("\n");
        ...
        SimulationLayer carLayer = new SimulationLayer(carCanvas, spawnerTop.getFxEntities()).addDrawable(spawnerBottom.getFxEntities());
