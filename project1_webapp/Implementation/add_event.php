...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql = "SELECT event_name FROM event WHERE event_name=:event_name";
    $stmt = $conn->prepare($sql);
    $stmt->bindParam(':event_name', strtolower($_POST['event_name']), PDO::PARAM_STR);
    ...
    try {
        ...
        if (!empty($result)) {
            ...
            header("location:/add_event.php?creation=eventnamealreadyexists");
            exit();
        }
    }
    ...
    if ($_POST['start_date'] > $_POST['end_date']) {
        ...
        header("location:/add_event.php?creation=startdatelaterthanenddate");
    }

    if (date("Y-m-d") > $_POST['start_date'] || (date("Y-m-d") > $_POST['end_date'])) {
        $errors[] = 'Start date and end date cannot be earlier than today';
        header("location:/add_event.php?creation=datenotlaterthantoday");
    ...
    if (count($errors) === 0 and isset($_SESSION['username'])) {    
    ...
    $stmt2->bindParam(":event_name", strtolower($_POST["event_name"]), PDO::PARAM_STR);   
    ...
    $stmt3->bindParam(":event_name", strtolower($_POST["event_name"]), PDO::PARAM_STR);     
    ...
    $stmt4->bindParam(":event_name", strtolower($_POST["event_name"]), PDO::PARAM_STR);     
    ...
    if (in_array($fileActualExt, $allowed)) {
        ...
        try {
            ...
            $sql69 = "INSERT INTO IMAGES values(:event_name, :image_id);"; //insert image reference into the database
            $upload_statement = $conn->prepare($sql69);
            $upload_statement->bindValue(':event_name', strtolower($_POST['event_name']), PDO::PARAM_STR);
        ...
        $success3 = false;
        try {
            $actually_successful3 = $stmt3->execute();

            $success3 = true;
        } catch (Exception $ex) {

        }
        if (!$actually_successful3) {
            var_dump($stmt3->errorInfo());
        }
        $success4 = false;
        try {
            $actually_successful4 = $stmt4->execute();

            $success4 = true;
        } catch (Exception $ex) {

        }
        if (!$actually_successful4) {
            var_dump($stmt4->errorInfo());
        }
    }
}
