                ...
                <h4>Change password</h4>        
                    ...
                    <?php 
                        ...
                        foreach ($events as $event):
                            $sql4 = "SELECT * FROM event_ticket_category WHERE event_name = '$event';";
                            $stmt4 = $conn-> prepare($sql4);
                            $stmt4->execute();
                            $etcs = $stmt4->fetchAll();

                            foreach ($etcs as $etc):
                                $category_id = $etc['etc_id'];
                                $sql5 = "SELECT count(id_category) as tickets_taken FROM TICKET where id_category = :id group by id_category ;";
                                $statement_amount_taken = $conn->prepare($sql5);
                                $statement_amount_taken->bindParam(':id', $category_id, PDO::PARAM_INT);
                                $statement_amount_taken->execute();
                                $category = $statement_amount_taken->fetch();                               
                        
                    ?>
                    
                    <p>
                       name:    <?php print htmlspecialchars($etc['event_name']);?> <br> 
                       category name: <?php print htmlspecialchars($etc['category']) ?> <br>
                       price:         <?php print htmlspecialchars($etc['price']); ?> <br>
                       amount taken:         <?php  print htmlspecialchars($category); ?><br>
                    </p>
                    <?php 
